...
print("🔥 LOADED PATIENTS.PY V3 - RESTORED AND FIXED")
...
async def get_me(
    ...
    if isinstance(current_patient, User):
        ...
        if not patient:
            # Handle legacy/new users who don't have a Patient record yet
            # Return a temporary Patient object with default values to satisfy the mobile app
            # This prevents 404 and prevents decoding errors
            return Patient(
                id=current_patient.id,  # Use User ID as temporary ID
                ...
                # Default values for Patient specific fields
                emergency_contacts=[],
                trigger_factors=[],
                medical_history="New patient",
                health_status="good",
                hashed_password=current_patient.hashed_password
            )
        return patient
